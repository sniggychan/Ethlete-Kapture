...
def analyze_vertical_jump(video_path):
    ...
    while True:
        ...
        if results.pose_landmarks:
            ...
            # Use midpoint of hips as vertical position
    ...
    if len(hip_y_positions) < 2:
        return {"jump_count": 0, "max_height_cm": 0, "flight_times": []}

    # Convert normalized y to pixel or relative height (assuming camera fixed)
    # Detect jump events by hip_y minima (lowest point) and maxima (highest point)
    hip_y_positions = np.array(hip_y_positions)
    timestamps = np.array(timestamps)

    # Invert y because in image coordinates, lower y means higher position
    hip_y_inv = 1 - hip_y_positions

    # Simple peak detection for jumps (local maxima)
    from scipy.signal import find_peaks

    peaks, _ = find_peaks(hip_y_inv, height=0.05, distance=fps*0.5)  # peaks at least 0.5s apart

    jump_count = len(peaks)
    flight_times = []

    # Estimate flight time between takeoff and landing (simplified)
    for i in range(jump_count - 1):
        flight_time = timestamps[peaks[i+1]] - timestamps[peaks[i]]
        flight_times.append(flight_time)

    # Calculate max jump height from flight time: h = g * (T/2)^2 / 2 = g*T^2/8
    g = 9.81  # m/s^2
    max_height_m = 0
    if flight_times:
        max_flight = max(flight_times)
        max_height_m = g * (max_flight ** 2) / 8

    max_height_cm = max_height_m * 100

    return {
        "jump_count": jump_count,
        "max_height_cm": round(max_height_cm, 1),
